"""
=============================================================================
 PIPELINE 4 — Storm Risk Detection (Binary Classification)
=============================================================================
 ✓ XGBoost Classifier
 ...
 Run:    python pipeline_4_storm.py
 Output: models/storm/
"""
...
def main():
    print("\n" + "=" * 65)
    print("  PIPELINE 4 — Storm Risk Detection")
    print("=" * 65)

    df = full_preprocess()
    features = get_features(df)
    print(f"  Features: {len(features)}  |  Storm rate: {df[TARGET].mean()*100:.2f}%")
    ...
    report = classification_report(y_test, preds, target_names=["Normal", "Storm"])
    ...
    if y_test.nunique() > 1:
        ...
        ax.set_title("Storm Risk — Reliability Diagram")
        ax.legend(); plt.tight_layout()
        plt.savefig(f"{OUT_DIR}/reliability_diagram.png", dpi=150); plt.close()

    fig, ax = plt.subplots(figsize=(8, 4))
    ax.hist(entropy, bins=50, color="#9b59b6", alpha=0.7, edgecolor="white")
    ax.set_xlabel("Entropy (bits)"); ax.set_ylabel("Count")
    ax.set_title("Storm Prediction Uncertainty")
    plt.tight_layout()
