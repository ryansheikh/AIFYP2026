...
@st.cache_data(show_spinner="🌐 Downloading 25 years of coastal weather data …")
def download_data():
    """Download from Open-Meteo API (free, no key needed)."""
    ...
    for name, (lat, lon) in STATIONS.items():
        r = requests.get(BASE, params={
            ...
            "daily": ",".join(DAILY_VARS), "timezone": "Asia/Karachi",
        }, timeout=120)
        r.raise_for_status()
...
def preprocess(df):
    ...
    # Lags
    for col in ["temperature_2m_mean","temperature_2m_max","precipitation_sum",
                "windspeed_10m_max","windgusts_10m_max","pressure_msl_mean"]:
    ...
    df["label_storm"]      = (df.get("windgusts_10m_max", df["windspeed_10m_max"]) >= 60).astype(int)
...
def main():
    ...
    if page == "🏠 Overview & Alerts":
        ...
        wg = dfs["windgusts_10m_max"].max() if "windgusts_10m_max" in dfs else 0
        ...
        sm = (dfs.get("windgusts_10m_max", dfs["windspeed_10m_max"])>=60).sum()
